...
fn main() -> Result<()> {
    ...
    // find_resource_categories(&client)?; // activate to retrieve all resource links
    
    let mut output = vec![];
    let resources = get_specific_resource(&client, "AWS_AppRunner.html")?;
    
    for r in resources {
        let (name, props) = retrieve_resource_props(&client, &r)?;
        let joined_props = props.into_iter().map(|v| {
            format!("{}={}", v.0, v.1.join(","))
        }).collect::<Vec<_>>().join(";");
...
fn retrieve_resource_props(client: &Client, path: &str) -> Result<(String, HashMap<String, Vec<String>>)> {
    ...
    let resource_html = client.get(&resource_url)
...
// retrieves a full list of resource urls (S3 for example) to check
// can print this to an output file to use later
#[allow(unused)]
fn find_resource_categories(client: &Client) -> Result<()> {
    let start_url = format!("{}/{}", BASE_URL, START_SUFFIX);
    let resource_html = client.get(&start_url)
        .header(USER_AGENT, "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36")
        .send()
