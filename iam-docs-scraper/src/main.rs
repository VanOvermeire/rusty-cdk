...
fn retrieve_service(client: &Client, link: &String) -> Result<String> {
    let service_html = client
        .get(format!("{}/{}", BASE_SERVICE_URL, link))
        .header(
            USER_AGENT,
    ...
    let service_code = service_code
        .next()
        .context(format!(
            "expected at least one <code> tag with the name of the service for {}",
            link
        ))?
        .inner_html();
