...
pub fn validate_doc_db_instance_class(value: &str) -> Result<(), String> {
    ...
    let instance_type_second_char = instance_type_second_char
        .to_string()
        .parse::<u8>()
        .map_err(|_| format!("document db instance type should be followed by a number (was {})", instance_type))?;

    if instance_type_first_char == 't' && instance_type_second_char < 3 {
        return Err(format!(
            "document db instance type should be of generation t3 or higher (was {})",
            instance_type_second_char
        ));
    }
    if instance_type_first_char == 'r' && instance_type_second_char < 4 {
        return Err(format!(
            "document db instance type should be of generation r4 or higher (was {})",
            instance_type_second_char
        ));
    }
    ...
    if !instance_size.ends_with("medium") && !instance_size.ends_with("large") {
        return Err(format!(
            "document db instance size should be `medium` or `(nx)large` (was {})",
            instance_size
        ));
    }
    ...
    if !instance_size_without_suffix.is_empty() && !VALID_INSTANCE_SIZES.contains(&instance_size_without_suffix.as_str()) {
        return Err(format!(
            "document db instance size of size large should have one of the following multipliers: {} (was {})",
            VALID_INSTANCE_SIZES.join(", "),
            instance_size
        ));
    }
